...
public class GitHubApiService extends Service {
    ...
    public void onCreate() {
        ...
        if (null == gitHubService) {
            ...
            gitHubService = ApiClientProvider.gitHubClient(authorization);
        }
    }

    public Observable<User> login(String username, String password) {
        String authCredential = Credentials.basic(username, password);
        gitHubService = ApiClientProvider.gitHubClient(authCredential);
    ...
    public Observable<User> user() {
        return gitHubService.user(null != user ? user.login : "");
    }

    public Observable<List<Repository>> starredRepositories() {
        return gitHubService.starredRepositories(null != user ? user.login : "");
    }

    public Observable<List<Repository>> repositories() {
        return gitHubService.usersRepositories(null != user ? user.login : "");
    }
